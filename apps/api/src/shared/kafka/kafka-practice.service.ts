...
export class KafkaPracticeService implements OnModuleInit, OnModuleDestroy {
  ...
  private async setupConsumers() {
    ...
    await this.kafkaConsumer.run(
      emailConsumerKey,
      async ({ key, value, topic, partition }) => {
    ...
    this.logger.log(
      `🔍 Active consumers: ${this.kafkaConsumer.getActiveConsumers().join(', ')}`,
    );
  }
  ...
  private async startSendingTestMessages() {
    ...
    for (let i = 0; i < orderEvents.length; i++) {
      setTimeout(async () => {
        ...
        this.logger.log(
          `📤 Sending order event: ${event.event} for ${event.orderId}`,
        );
  ...
  private async simulateEmailSending(message: any) {
    ...
    this.logger.log(
      `✉️ Email sent for order ${message.orderId} (${message.event})`,
    );
  }
  ...
  private async simulateSMSSending(message: any) {
    ...
    this.logger.log(
      `📲 SMS sent for order ${message.orderId} (${message.event})`,
    );
  }
  ...
  private async simulateAnalyticsRecording(message: any) {
    ...
    this.logger.log(
      `📈 Analytics recorded for order ${message.orderId} (${message.event})`,
    );
  }
  ...
  private async simulateInventoryUpdate(message: any) {
    ...
    this.logger.log(
      `📋 Inventory updated for order ${message.orderId} (${message.event})`,
    );
  }
  ...
  async triggerOrderCreated(orderId: string, userId: string, total: number) {
    ...
    this.logger.log(
      `🎯 Manually triggering ORDER_CREATED event for ${orderId}`,
    );
    await this.kafkaProducer.send({
      topic: 'order-events',
      messages: [
